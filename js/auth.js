...
const AVATARS_STORAGE_KEY = 'forge-ai-avatars'; // { [email]: 'data:image/...;base64,...' }
...
const Auth = {
    ...
    // ---- Foto de perfil (guardada como data URL en base64, por correo) ----
    getAllAvatars() {
        try {
    ...
    getAvatar(email) {
        ...
        return avatars[email.trim().toLowerCase()] || null;
    },

    setAvatar(email, dataUrl) {
        if (!email) return;
        const avatars = this.getAllAvatars();
        avatars[email.trim().toLowerCase()] = dataUrl;
    ...
    removeAvatar(email) {
        if (!email) return;
        const avatars = this.getAllAvatars();
        delete avatars[email.trim().toLowerCase()];
        localStorage.setItem(AVATARS_STORAGE_KEY, JSON.stringify(avatars));
    }
};
