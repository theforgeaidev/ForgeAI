...
var AVATAR_MAX_BYTES = 5 * 1024 * 1024; // 5MB
var AVATAR_ALLOWED_TYPES = /^image\/(png|jpe?g|webp)$/;

// Pinta la imagen (o la inicial, si no hay foto) dentro de cada elemento .side-profile-avatar / .settings-avatar
function renderAvatarInto(selector, user) {
    document.querySelectorAll(selector).forEach(function (el) {
        var avatarUrl = user && user.email ? Auth.getAvatar(user.email) : null;
        if (avatarUrl) {
            el.innerHTML = '<img src="' + avatarUrl + '" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:inherit;display:block;">';
        } else {
...
function updateAvatarUI() {
    ...
    renderAvatarInto('.side-profile-avatar', user);
    renderAvatarInto('.settings-avatar', user);
}
window.updateAvatarUI = updateAvatarUI;

function handleAvatarFile(file) {
    if (!file) return;
    if (!AVATAR_ALLOWED_TYPES.test(file.type) || file.size > AVATAR_MAX_BYTES) {
        alert(I18n.t('profile.photoError'));
        return;
    }
    ...
    reader.onload = function () {
        var user = Auth.getUser();
        if (!user) return;
        Auth.setAvatar(user.email, reader.result);
        updateAvatarUI();
    };
    reader.readAsDataURL(file);
}

// Convierte un elemento avatar en un botón clicable que abre el selector de archivos
function makeAvatarClickable(el, hiddenInput) {
    if (!el || el.dataset.avatarBound) return;
    el.dataset.avatarBound = '1';
    el.style.cursor = 'pointer';
    el.title = I18n.t('profile.changePhoto');
...
function setupAvatarUpload() {
    ...
    sharedInput.accept = 'image/png,image/jpeg,image/webp';
    ...
    document.querySelectorAll('.side-profile-avatar, .settings-avatar').forEach(function (el) {
        makeAvatarClickable(el, sharedInput);
    });

    // Debajo del avatar grande de Configuración, añade un enlace de texto
    // "Cambiar foto \u00b7 Quitar foto" (se inserta una sola vez).
    var settingsAvatar = document.querySelector('.settings-avatar');
    if (settingsAvatar && !document.getElementById('avatar-actions-row')) {
        var row = document.createElement('div');
        row.id = 'avatar-actions-row';
        row.style.cssText = 'font-size:11px;margin-top:6px;display:flex;gap:8px;color:#71717a;';
        row.innerHTML = '<button type="button" id="avatar-change-link" style="color:#a1a1aa;text-decoration:underline;background:none;border:none;cursor:pointer;padding:0;">' + I18n.t('profile.changePhoto') + '</button>' +
            '<button type="button" id="avatar-remove-link" style="color:#f87171;text-decoration:underline;background:none;border:none;cursor:pointer;padding:0;">' + I18n.t('profile.removePhoto') + '</button>';
        var parent = settingsAvatar.closest('.flex') || settingsAvatar.parentElement;
        if (parent && parent.parentElement) {
            parent.parentElement.insertBefore(row, parent.nextSibling);
        }
        document.getElementById('avatar-change-link').addEventListener('click', function (e) { e.stopPropagation(); sharedInput.click(); });
        document.getElementById('avatar-remove-link').addEventListener('click', function (e) {
            e.stopPropagation();
            var user = Auth.getUser();
            if (user) { Auth.removeAvatar(user.email); updateAvatarUI(); }
        });
    }
}
...
function switchSettingsPanel(panelName) {
    ...
    // Update title (traducido según el idioma activo)
    var titleKeys = { general: 'settings.nav.general', account: 'settings.nav.account', privacy: 'settings.nav.privacy', appearance: 'settings.nav.language', data: 'settings.nav.data' };
    var titleEl = document.getElementById('settings-panel-title');
    if (titleEl) titleEl.textContent = I18n.t(titleKeys[panelName] || 'settings.nav.general');
}

var LANGUAGE_LABELS = { en: 'English', es: 'Espa\u00f1ol', fr: 'Fran\u00e7ais', pt: 'Portugu\u00eas' };
...
function getTypewriterPhrases() {
    var list = I18n.t('main.typewriter');
    return (Array.isArray(list) && list.length) ? list : ['Ask Forge AI...'];
}
...
function startTypewriter() {
    ...
    twIndex = 0; twIsDeleting = false;
    var currentText = getTypewriterPhrases()[twIndex];
    twCharIndex = Math.floor(currentText.length / 3);
    typeWriterTick();
}
function typeWriterTick() {
    if (isTyping) return;
    var phrases = getTypewriterPhrases();
...
// Cuando cambia el idioma (desde cualquier selector, incluido el del modal de
// Configuración), reinicia el typewriter con las frases del idioma nuevo.
document.addEventListener('forge:languagechange', function () {
    if (promptInput && promptInput.value.length === 0) startTypewriter();
});
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    setupAvatarUpload(); updateAvatarUI();
